...
def make_signal(start_time, end_time, values):
    ...
    for i in range(len(values)):
        signal.set_value_at_time(i + start_time, values[i])
    return signal
...
def impulse_moving_average(length):
    values = [1 / length] * length
    return make_signal(0, length - 1, values)
...
def max_absolute_difference(first_signal:DiscreteSignal, second_signal:DiscreteSignal):
    ...
    for i in range(start, end + 1):
        diff = max(diff, abs(first_signal.get_value_at_time(i) - second_signal.get_value_at_time(i)))
    return diff
...
# py main.py inputs/1.txt --out-dir outputs/1
# py main.py inputs/2.txt --out-dir outputs/2
# py main.py inputs/3.txt --out-dir outputs/3
# py main.py inputs/4.txt --out-dir outputs/4