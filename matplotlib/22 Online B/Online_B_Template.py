...
def stem_plot(ax, n, x, c, label):
    """A nicer stem plot for discrete-time sequences."""
    markerline, stemlines, baseline = ax.stem(n, x, linefmt = c, label=label)
...
def main():
    ...
    stem_plot(ax1, n, x, f"r", "original x[n]")
    stem_plot(ax1, n, x_time, f"g", f"time shift by n0={n0}")
    stem_plot(ax1, n, x_phase_equiv, f"b", f"phase change by phi0={phi0_equiv:.3f}")
    # ax1.plot(n,x,'o',linestyle = ' ', color = 'r', label = 'original')
    # ax1.plot(n,x_time,'o',linestyle = ' ', color = 'b', label = 'xtime')
    # ax1.plot(n,x_phase_equiv,'o',linestyle = ' ',color = 'g', label = 'xphase')
    ax1.legend()
    fig1.tight_layout()
    fig1.show()
    # return
    
    phi0 = 55  # an arbitrary phase change
    ...
    stem_plot(ax2, n, x_phase, f"r", f"phase change by phi0={phi0:.3f}")
    stem_plot(ax2, n, x_time_best, f"g",f"best time shift k={best_k}")
    ax2.legend()
    fig2.tight_layout()
    plt.show()
