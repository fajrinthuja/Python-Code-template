...
class CFT2D:
    ...
    def compute_cft(self):
        ...
        A = np.trapezoid(self.I[:, :, None] * cos_ux[None, :, :], x=self.x, axis=1)
        B = np.trapezoid(self.I[:, :, None] * sin_ux[None, :, :], x=self.x, axis=1)
        cos_vy = np.cos(2 * np.pi * self.v[:, None] * self.y[None, :])
        sin_vy = np.sin(2 * np.pi * self.v[:, None] * self.y[None, :])
        term1 = np.trapezoid(A[:, None, :] * cos_vy[:, :, None], x=self.y, axis=0)
        term2 = np.trapezoid(B[:, None, :] * sin_vy[:, :, None], x=self.y, axis=0)
        term3 = np.trapezoid(A[:, None, :] * sin_vy[:, :, None], x=self.y, axis=0)
        term4 = np.trapezoid(B[:, None, :] * cos_vy[:, :, None], x=self.y, axis=0)
        real = term1 - term2
        imag = -(term3 + term4)
        return real, imag
...
class InverseCFT2D:
    ...
    def reconstruct(self):
        ...
        integrand_C = (self.real[None, :, :] * cos_vy[:, :, None] - self.imag[None, :, :] * sin_vy[:, :, None])
        integrand_D = (self.real[None, :, :] * sin_vy[:, :, None] + self.imag[None, :, :] * cos_vy[:, :, None])
        C = np.trapezoid(integrand_C, x=self.v, axis=1) 
        D = np.trapezoid(integrand_D, x=self.v, axis=1)
        cos_ux = np.cos(2 * np.pi * self.x[:, None] * self.u[None, :])
        sin_ux = np.sin(2 * np.pi * self.x[:, None] * self.u[None, :])
        integrand_reconstruct = (C[:, None, :] * cos_ux[None, :, :] - D[:, None, :] * sin_ux[None, :, :])
        image = np.trapezoid(integrand_reconstruct, x=self.u, axis=2)
        return image
