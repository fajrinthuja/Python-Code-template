...
def convolve_plane(plane, kernel, engine, circular=False):
    ...
    if circular:
        ...
        # Wrap kernel around origin to align center with (0, 0)
    ...
    # Crop the valid centered (H, W) region
    r_start = kh // 2
    c_start = kw // 2
...
def run_single(path, kernel_name, param, engine_name, out_dir, gray=False):
    ...
    # 1. save_image(array, path)
    save_image(blurred, blurred_path)
    save_image(wraparound, wrap_path)

    # 2. save_kernel_preview(kernel, out_path)
    save_kernel_preview(kernel, kernel_path)

    # 3. save_comparison(images, titles, out_path)
    save_comparison(
